...
public sealed class ObservabilityConfigurationTests
{
		...
		ObservabilityTestEnvironment.WithClearedOtelEnvironment(() =>
																{
																	...
																	host.Services
																		.GetServices<TracerProvider>()
																		.Should()
																		.BeEmpty();
																	host.Services
																		.GetServices<MeterProvider>()
																		.Should()
																		.BeEmpty();
																	host.Services
																		.GetServices<LoggerProvider>()
																		.Should()
																		.BeEmpty();
																});
		...
		ObservabilityTestEnvironment.WithClearedOtelEnvironment(() =>
																{
																	...
																	host.Services
																		...
																		.Should()
																		.Contain(provider => provider.GetType().Name == "ConsoleLoggerProvider");
																});
	...
	public void StandardOpenTelemetrySettingsPassThroughWithoutMonacoValidationOrMutation()
	{
		...
		builder.Configuration["OTEL_EXPORTER_OTLP_ENDPOINT"]
			   .Should()
			   .Be("consumer-selected-endpoint");
		builder.Configuration["OTEL_TRACES_SAMPLER"]
			   .Should()
			   .Be("consumer-selected-sampler");
		builder.Configuration["OTEL_BSP_MAX_QUEUE_SIZE"]
			   .Should()
			   .Be("consumer-selected-queue");
		builder.Configuration["OTEL_DOTNET_EXPERIMENTAL_OTLP_RETRY"]
			   .Should()
			   .Be("consumer-selected-retry");
	}
	...
	public void HostCompositionCapturesOneImmutableSignalSnapshot()
	{
		...
		using var host = builder.Build();
		host.Services
			.GetRequiredService<IOptions<ObservabilityStartupOptions>>()
			.Value
			.TracesEnabled
			.Should()
			.BeFalse();
	...
	public async Task AuthenticatedRequestsSelectFirstNonEmptyValidatedSubjectAndAttachRawClaimsToEntrySpanAndLogScope()
	{
		...
		[
			new("sub",
				string.Empty,
				"empty-subject-value-type",
				"empty-subject-issuer",
				"empty-subject-original-issuer"),
			new("role",
				"administrator",
				"role-value-type",
				"role-issuer",
		...
		var context = new DefaultHttpContext { User = new ClaimsPrincipal(new ClaimsIdentity(claims, "validated")) };
		...
		nextCalled.Should()
				  .BeTrue();
		activity.GetTagItem("user.id")
				.Should()
				.Be(subject);
		AssertClaimEvents(activity, expectedClaims);

		var scope = logger.Scopes
						  .Should()
						  .ContainSingle()
						  .Which;
		scope.Should()
			 .Contain(new KeyValuePair<string, object?>("user.id", subject));
		GetScopedClaims(scope)
			.Should()
			.Equal(expectedClaims);

		var downstreamScope = logger.Entries
									.Should()
									.ContainSingle()
									.Which
									.Scopes
									...
									.ContainSingle()
									.Which;
		downstreamScope.Should()
					   .Contain(new KeyValuePair<string, object?>("user.id", subject));
		GetScopedClaims(downstreamScope)
			.Should()
			.Equal(expectedClaims);
	}
	...
	public async Task AuthenticatedClientRequestsWithoutSubjectRetainRawClaimsWithoutUserId()
	{
		...
		var scope = logger.Scopes
						  ...
						  .Which;
		scope.Should()
			 .NotContain(field => field.Key == "user.id");
		GetScopedClaims(scope)
			.Should()
			.Equal(expectedClaims);
	...
	[
		.. claims.Select(claim => new IdentityEnrichmentMiddleware.ValidatedClaimContext(claim.Type,
																						 claim.Value,
																						 claim.ValueType,
																						 claim.Issuer,
	...
	{
		var claimEvents = activity.Events
								  .Where(@event => @event.Name == "user.claim")
								  .ToArray();
		claimEvents.Should()
				   .HaveCount(expectedClaims.Length);
	...
	{
		var value = scope.Should()
						 .ContainSingle(field => field.Key == "user.claims")
						 .Which
						 .Value;
		value.Should()
			 .BeOfType<string>();
	...
	public async Task BoundaryExceptionHandlingLogsOriginalExceptionWithoutMutatingActivity()
	{
		...
		logger.Entries
			  .Should()
			  .ContainSingle(entry => entry.LogLevel == LogLevel.Error && ReferenceEquals(entry.Exception, exception));
		activity.GetTagItem("exception.type")
				.Should()
				.Be("existing");
		activity.Events
				.Should()
				.ContainSingle(@event => @event.Name == "existing-event");
	}

	private static IConfiguration CreateConfiguration(params (string Key, string? Value)[] values) =>
		new ConfigurationBuilder().AddInMemoryCollection(values.ToDictionary(value => value.Key, value => value.Value))
								  .Build();
	...
	private sealed class CapturedScope : IDisposable
	{
		private readonly List<IReadOnlyCollection<KeyValuePair<string, object?>>> _activeScopes;

		public CapturedScope(List<IReadOnlyCollection<KeyValuePair<string, object?>>> activeScopes) =>
