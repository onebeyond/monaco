...
using DcslGs.Template.Application.Validators;
...
namespace DcslGs.Template.Application.Tests.Validators;

[ExcludeFromCodeCoverage]
public class CompanyCreateCommandValidatorTests
{
	[Trait("Application Validators", "Company Validators")]
	[Fact(DisplayName = "Validator's cascade mode is 'Stop'")]
	public void ValidatorCascadeModeIsStop()
	{
		var sut = new CompanyCreateCommandValidator(new Mock<AppDbContext>().Object);

		sut.CascadeMode.Should().Be(CascadeMode.Stop);
	}
	...
	public async Task NameDoesNotGenerateErrorWhenValid()
	{
		...
		var companyDbSetMock = new List<Company>().AsQueryable().BuildMockDbSet();
		dbContextMock.Setup(x => x.Set<Company>()).Returns(companyDbSetMock.Object);
	...
	[AnonymousData]
	public async Task NameAlreadyExistsGeneratesError(Company company)
	{
		var dbContextMock = new Mock<AppDbContext>();

		var companyDbSetMock = new List<Company> { company }.AsQueryable().BuildMockDbSet();
		dbContextMock.Setup(x => x.Set<Company>()).Returns(companyDbSetMock.Object);
	...
	[AnonymousData(true)]
	public async Task CountryIsValidDoesNotGenerateError(Country country)
	{
		var dbContextMock = new Mock<AppDbContext>();
		...
		dbContextMock.Setup(x => x.Set<Country>()).Returns(countryDbSetMock.Object);
	...
	[AnonymousData]
	public async Task CountryMustExistValidation(Country country)
	{
		var dbContextMock = new Mock<AppDbContext>();
